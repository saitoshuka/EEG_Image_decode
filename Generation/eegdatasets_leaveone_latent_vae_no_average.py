...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None):
        ...
        self.exclude_subject = exclude_subject  # Save this parameter
    ...
    def load_data(self):
        ...
        for dir in dirnames:
            # Try to find the position of the first '_'
            try:
                idx = dir.index('_')
        ...
        if self.train:
            img_directory = img_directory_training  # Replace with your new path
        else:
            img_directory = img_directory_test
        
        all_folders = [d for d in os.listdir(img_directory) if os.path.isdir(os.path.join(img_directory, d))]
        all_folders.sort()  # Ensure folder order
        ...
        if self.classes is not None and self.pictures is not None:
            ...
            # Handle other cases, e.g., when self.classes and self.pictures lengths don't match
            print("Error")
        ...
        for subject in self.subjects:
            if self.train:
                if subject == self.exclude_subject:  # Skip the excluded subject
                    continue            
                ...
                ch_names = data['ch_names']  # Keep as Python list or encode appropriately
                ...
                if self.classes is not None and self.pictures is not None:
                    for c, p in zip(self.classes, self.pictures):
                        start_index = c * 1 + p
                        if start_index < len(preprocessed_eeg_data):  # Ensure index doesn't exceed range
                            preprocessed_eeg_data_class = preprocessed_eeg_data[start_index: start_index+1]  # Select only one data sample
                            labels = torch.full((1,), c, dtype=torch.long).detach()  # Add class labels
                    ...
                    for c in self.classes:
                        ...
                        labels = torch.full((samples_per_class,), c, dtype=torch.long).detach()  # Add class labels
                        data_list.append(preprocessed_eeg_data_class)
                ...
                if subject == self.exclude_subject or self.exclude_subject==None:  # Skip the excluded subject
                    ...
                    samples_per_class = 1  # Each class has 1 sample
    ...
    def Textencoder(self, text):   
            # Use preprocessor to convert text to model input format
            text_inputs = torch.cat([clip.tokenize(t) for t in text]).to(device)
            # print("text_inputs", text_inputs)
            # Use CLIP model to encode text
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    # Training EEG data: torch.Size([16540, 4, 17, 100]) [num_training_images, num_repetitions, num_channels, num_timepoints]
    # Test EEG data: torch.Size([200, 80, 17, 100])
